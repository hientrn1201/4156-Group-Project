...
/**
 * Service for chunking document text into smaller, manageable pieces
 */
@Service
public class DocumentChunkingService {
     ...
     */
    @Transactional
    public List<DocumentChunk> chunkDocument(Document document, Integer chunkSize, Integer overlapSize) {
        if (document == null || document.getExtractedText() == null || document.getExtractedText().trim().isEmpty()) {
        ...
        while (startIndex < text.length()) {
            ...
            if (endIndex < text.length()) {
                ...
                if (lastSentenceEnd > actualChunkSize * 0.7) { // Only break at sentence if it's not too short
    ...
    public ChunkStatistics getChunkStatistics(Document document) {
        ...
        return new ChunkStatistics(totalChunks, totalCharacters, averageChunkSize, minChunkSize, maxChunkSize);
    }
    ...
    public static class ChunkStatistics {
        ...
        public ChunkStatistics(int totalChunks, int totalCharacters, int averageChunkSize, int minChunkSize,
                int maxChunkSize) {
