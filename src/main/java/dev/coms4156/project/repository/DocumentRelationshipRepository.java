...
public interface DocumentRelationshipRepository extends JpaRepository<DocumentRelationship, Long> {
    ...
    List<DocumentRelationship> findByRelationshipType(DocumentRelationship.RelationshipType relationshipType);

    long countByRelationshipType(DocumentRelationship.RelationshipType relationshipType);

    @Query("SELECT dr FROM DocumentRelationship dr WHERE dr.sourceChunk.document.id = :documentId OR dr.targetChunk.document.id = :documentId")
    List<DocumentRelationship> findByDocumentId(@Param("documentId") Long documentId);

    @Query("SELECT dr FROM DocumentRelationship dr WHERE dr.similarityScore >= :threshold ORDER BY dr.similarityScore DESC")
    List<DocumentRelationship> findBySimilarityScoreGreaterThanEqual(@Param("threshold") Double threshold);
}
