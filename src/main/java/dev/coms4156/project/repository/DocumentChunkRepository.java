...
public interface DocumentChunkRepository extends JpaRepository<DocumentChunk, Long> {
    ...
    @Query("SELECT dc FROM DocumentChunk dc WHERE dc.document = :document ORDER BY dc.chunkIndex")
    ...
    @Query(value = "SELECT * FROM document_chunks WHERE document_id = :documentId AND embedding IS NOT NULL", nativeQuery = true)
    List<DocumentChunk> findChunksWithEmbeddingsByDocumentId(@Param("documentId") Long documentId);

    @Query(value = "SELECT * FROM document_chunks WHERE document_id = :documentId AND embedding IS NULL", nativeQuery = true)
    ...
    @Modifying
    @Query(value = "INSERT INTO document_chunks (chunk_index, chunk_size, document_id, embedding, end_position, start_position, text_content) VALUES (:chunkIndex, :chunkSize, :documentId, CAST(:embedding AS vector), :endPosition, :startPosition, :textContent)", nativeQuery = true)
